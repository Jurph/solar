...
class UniverseExporter:
    ...
    def export_universe(
        self, galaxy_filter: Optional[str] = None, system_filter: Optional[str] = None
    ) -> str:
        """
    ...
    def export_galaxy(
        self, galaxy: Galaxy, system_filter: Optional[str] = None
    ) -> ET.Element:
    ...
    def export_system(self, system: StarSystem, parent: ET.Element) -> ET.Element:
        ...
        if getattr(system, "system_age_years", None) is not None:
            ET.SubElement(system_elem, "system_age_years").text = str(
                system.system_age_years
            )
    ...
    def export_star(self, star: Star, parent: ET.Element) -> ET.Element:
        ...
        ET.SubElement(star_elem, "magnitude").text = str(
            getattr(star, "star_magnitude", "0")
        )
        ...
        if getattr(star, "luminosity_solar", None) is not None:
            ET.SubElement(star_elem, "luminosity_solar").text = str(
    ...
    def export_planet(self, planet: Planet, parent: ET.Element) -> ET.Element:
        ...
        if getattr(planet, "equilibrium_temperature_k", None) is not None:
            ET.SubElement(planet_elem, "equilibrium_temperature_k").text = str(
                planet.equilibrium_temperature_k
            )
        if getattr(planet, "orbital_eccentricity", None) is not None:
            ET.SubElement(planet_elem, "orbital_eccentricity").text = str(
                planet.orbital_eccentricity
            )
        if getattr(planet, "orbital_inclination_deg", None) is not None:
            ET.SubElement(planet_elem, "orbital_inclination_deg").text = str(
                planet.orbital_inclination_deg
            )

        # Atmosphere (ContentType-based; export if present)
        try:
            atmosphere = (
                planet.get_atmosphere() if hasattr(planet, "get_atmosphere") else None
            )
        except Exception:
            atmosphere = None
        if atmosphere:
    ...
    def export_moon(self, moon: Moon, parent: ET.Element) -> ET.Element:
        ...
        if getattr(moon, "orbital_distance_km", None) is not None:
            ET.SubElement(moon_elem, "orbital_distance_km").text = str(
                moon.orbital_distance_km
            )
        if getattr(moon, "orbital_period_hours", None) is not None:
            ET.SubElement(moon_elem, "orbital_period_hours").text = str(
                moon.orbital_period_hours
            )
        if getattr(moon, "rotation_period_hours", None) is not None:
            ET.SubElement(moon_elem, "rotation_period_hours").text = str(
        ...
        if getattr(moon, "is_tidally_locked", None) is not None:
            ET.SubElement(moon_elem, "is_tidally_locked").text = (
                "true" if moon.is_tidally_locked else "false"
            )
        ...
        try:
            atmosphere = (
                moon.get_atmosphere() if hasattr(moon, "get_atmosphere") else None
            )
        except Exception:
            atmosphere = None
        if atmosphere:
    ...
    def export_stations(self, parent_body: Location, parent_elem: ET.Element) -> None:
        ...
        for station in Station.objects.filter(orbits=parent_body):
            ...
            if getattr(station, "orbital_distance_km", None) is not None:
                ET.SubElement(station_elem, "orbital_distance_km").text = str(
                    station.orbital_distance_km
                )
            if getattr(station, "orbit_type", None) is not None:
                ET.SubElement(station_elem, "orbit_type").text = station.orbit_type
            ET.SubElement(station_elem, "large_berths").text = str(
                getattr(station, "large_berths", 0)
            )
            ET.SubElement(station_elem, "medium_berths").text = str(
                getattr(station, "medium_berths", 0)
            )
            ET.SubElement(station_elem, "small_berths").text = str(
                getattr(station, "small_berths", 0)
            )
