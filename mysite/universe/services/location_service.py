...
def get_subordinate_bodies(location: Optional[Location] = None, location_name: Optional[str] = None) -> List[Location]:
    """
...
def get_distance_between(location1: Location, location2: Location) -> Tuple[Optional[float], str]:
    """
    ...
    if isinstance(concrete1, Planet) and isinstance(concrete2, Planet):
        if parent1 and parent2 and parent1 == parent2:
            ...
            if dist1_au is not None and dist2_au is not None:
                distance_au = _calculate_planet_to_planet_distance(concrete1, concrete2, dist1_au, dist2_au)
    ...
    if parent1 and parent2 and parent1 != parent2:
        ...
        if isinstance(concrete1, (Station, Moon)) and isinstance(concrete2, (Station, Moon)):
...
def _calculate_planet_to_planet_distance(planet1: Planet, planet2: Planet, 
                                         dist1_au: float, dist2_au: float) -> Optional[float]:
    """
...
def _get_orbital_distance_from_parent(location, parent: Location) -> Optional[float]:
    ...
    if isinstance(location, (Planet, Moon)):
        if hasattr(location, 'orbital_distance_au') and location.orbital_distance_au is not None:
            return location.orbital_distance_au
        elif hasattr(location, 'orbital_distance_km') and location.orbital_distance_km is not None:
    ...
    if isinstance(location, Station):
        ...
        if hasattr(location, 'orbital_distance_km') and location.orbital_distance_km is not None:
            # Convert km to AU (1 AU ≈ 1.496e8 km)
            return location.orbital_distance_km / 1.496e8
...
def _calculate_same_parent_distance(location1, location2, parent: Location) -> Tuple[Optional[float], str]:
    """
    ...
    if hasattr(location1, 'orbital_distance_au') and location1.orbital_distance_au is not None:
        dist1_au = location1.orbital_distance_au
    elif hasattr(location1, 'orbital_distance_km') and location1.orbital_distance_km is not None:
        # Convert km to AU (1 AU ≈ 1.496e8 km)
        dist1_au = location1.orbital_distance_km / 1.496e8
    
    # Try to get orbital distance for location2
    if hasattr(location2, 'orbital_distance_au') and location2.orbital_distance_au is not None:
        dist2_au = location2.orbital_distance_au
    elif hasattr(location2, 'orbital_distance_km') and location2.orbital_distance_km is not None:
        # Convert km to AU (1 AU ≈ 1.496e8 km)
...
def get_distance_between_star_systems(system1: StarSystem, system2: StarSystem) -> float:
    """
    ...
    """
    if not all([system1.galactic_x_ly is not None, system1.galactic_y_ly is not None, 
               system1.galactic_z_ly is not None]):
        return float('inf')
    if not all([system2.galactic_x_ly is not None, system2.galactic_y_ly is not None,
