...
class ScriptService:
    ...
    def parse_navigation_event(self, nav_event: NavigationEvent, ship: Ship) -> DialogueEvent:
        ...
        if nav_event.maneuver == ManeuverType.SUBLIGHT:
            pilot = ship.pilot
            # Determine control name from the nav event; default to "[destination name] Control" if missing.
            controller_name = getattr(nav_event.controller, "name", None) or f"{nav_event.destination.name} Control"
            text = (
                f"{controller_name}, this is {self.pilot_call_sign}, inbound from {nav_event.origin.name} "
                f"carrying {self.default_cargo}. I need a vector for an insertion burn for {nav_event.destination.name} orbit."
            )
