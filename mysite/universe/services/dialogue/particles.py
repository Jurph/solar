...
class PilotRequest(DialogueParticle):
    ...
    def format_user_prompt(self, data: UserPromptData) -> str:
        ...
        for i, line in enumerate(lines):
            if line.startswith("Below is the LAST_DIALOGUE_LINE") or line.startswith(
                "Here are three GOOD example messages"
            ):
...
class RadioResponse(DialogueParticle):
    ...
    def get_examples(self) -> List[str]:
        ...
        physics_params = {}
        if self.actor:
            physics_params = ControllerPhysicsService.generate_parameters(
                self.actor, self.nav_context
            )
        ...
        # Departure maneuvers (sublight, transfer, hyperspace)
            ...
            azimuth_deg = physics_params.get("azimuth_deg") or physics_params.get(
                "departure_angle_deg"
            )
        ...
        # Deorbit
            ...
            if self.actor and self.actor.location:
                from mysite.universe.services.controller_physics import (
                    ControllerPhysicsService,
                )

                body = ControllerPhysicsService.get_relevant_body(
                    self.actor, self.nav_context
                )
                if body:
        ...
        if (
            self.nav_context.get("after_hold", False)
            or "adjusted" in str(self.nav_context.get("maneuver_type", "")).lower()
        ):
            examples.extend(
                [
...
class RadioReadback(DialogueParticle):
    ...
    def get_examples(self) -> List[str]:
        ...
        examples.append(
            f"We copy. {altitude_km} kilometers, {inclination_deg} degrees. Proceeding with {maneuver}."
        )
        examples.append(
            f"Roger, {maneuver} to {altitude_km} kilometers at {inclination_deg} degrees."
        )
...
class NavBroadcast(DialogueParticle):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._cached_broadcast_data = (
            None  # Cache for consistent data between announcement and modem encoding
        )
    ...
    def get_examples(self) -> List[str]:
        ...
        subordinate_bodies = (
            get_subordinate_bodies(location=location) if location else []
        )
    ...
    def generate_nav_broadcast_text(self) -> str:
        ...
        if hazard_info:
            ...
            hazard_text = (
                f"one Class {hazard_info['hazard_class']} {hazard_info['hazard']}"
            )
