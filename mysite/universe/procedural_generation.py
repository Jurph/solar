...
class SeededRandom:
    ...
    def choices(
        self, population: List[Any], weights: Optional[List[float]] = None, k: int = 1
    ) -> List[Any]:
    ...
    def triangular(
        self, low: float, high: float, mode: Optional[float] = None
    ) -> float:
...
def normal_clamped(
    rng: SeededRandom, mu: float, sigma: float, min_val: float, max_val: float
) -> float:
    """
...
def bin_select_weighted(
    rng: SeededRandom, bins: List[Tuple[float, float]], weights: List[float]
) -> float:
    """
...
STAR_TYPE_MASS_RANGES = {  # In solar masses
    ...
    "M": (
        0.08,
        0.45,
    ),  # Baraffe et al. 2015 - Lower limit is hydrogen burning threshold
}
...
def calculate_star_radius_from_mass_density(
    mass_kg: float, density_kg_m3: float
) -> float:
    """
...
def generate_planet_type(
    rng: SeededRandom, orbital_distance_au: float, star_type: str = "G"
) -> str:
    """
        ...
        if dist_min <= orbital_distance_au < dist_max:
            ...
            if star_type in ["M", "K"]:  # Cooler stars
                ...
                if "GG" in adjusted_weights:
                    ...
                    adjusted_weights["TE"] = (
                        adjusted_weights.get("TE", 0) + gg_weight * 0.3
                    )
                    adjusted_weights["SE"] = (
                        adjusted_weights.get("SE", 0) + gg_weight * 0.2
                    )
...
def generate_composition(rng: SeededRandom, planet_type: str) -> Dict[str, Any]:
    ...
    """
    ranges = COMPOSITION_RANGES_BY_TYPE.get(
        planet_type, COMPOSITION_RANGES_BY_TYPE["TE"]
    )
...
def calculate_density_from_composition(
    rng: SeededRandom, composition: Dict[str, Any], planet_type: str = None
) -> float:
    """
    ...
    if planet_type in ["GG", "IG"]:
        ...
        if planet_type == "GG":
            base_density = (
                800  # Jupiter ~1326, Saturn ~687, but we'll use lower for variation
            )
        else:  # IG
            base_density = 1500  # Neptune ~1638, Uranus ~1270
        # Core density from composition (for variation)
        core_density = iron_content * DENSITY_IRON + (1.0 - iron_content) * DENSITY_ROCK
        ...
        if water_coverage > 0.5:
            ...
            water_effect = (
                water_coverage - 0.5
            ) * 0.01  # Max 0.5% reduction at 100% coverage
...
def generate_albedo(
    ...
    rock_fraction = max(
        0.0, 1.0 - ice_weight - water_weight - iron_content - carbon_content
    )
    ...
    total_weight = (
        ice_weight + water_weight + rock_fraction + iron_weight + carbon_weight
    )
    if total_weight > 0:
        base_albedo = (
...
MOON_COMPOSITION_RANGES_BY_VARIETY = {
    "R": {  # Rocky (Luna-like)
        "iron_content": (
            0.08,
            0.18,
        ),  # Lower than planets - small moons don't form large cores
        "ice_content": (0.0, 0.05),
        "has_methane": False,
        "has_sulfur": (0.0, 0.1),
        "water_coverage": (0.0, 0.0),
...
def generate_moon_composition(rng: SeededRandom, moon_variety: str) -> Dict[str, Any]:
    ...
    Returns:
        Dictionary with composition values
    """
    ranges = MOON_COMPOSITION_RANGES_BY_VARIETY.get(
        moon_variety, MOON_COMPOSITION_RANGES_BY_VARIETY["R"]
    )
...
MOON_QUANTITY_BY_PARENT = {
    "STAR": (
        ...
        10,
        0,
    ),  # Stars rarely have moons (Pluto-type objects are usually classified as planets)
    "TE": (
        0,
        2,
        0,
    ),  # Terrestrial planets: 0-2 moons (Earth: 1, Mars: 2, Venus: 0, Mercury: 0)
...
MOON_TYPE_WEIGHTS_BY_PARENT_AND_DISTANCE = {
    ...
    "GG": {  # Gas giants
        (0.0, 0.1): {
            ...
            "O": 0.08,
            "T": 0.02,
        },  # Close: rocky/icy (Io, Europa)
        (0.1, 0.5): {
            "R": 0.2,
            "I": 0.6,
            "O": 0.15,
            "T": 0.05,
        },  # Medium: icy/organic (Ganymede, Callisto)
        (0.5, 2.0): {
            "R": 0.1,
            "I": 0.5,
            "O": 0.3,
            "T": 0.1,
        },  # Far: organic/terrestrial (Titan)
        (2.0, float("inf")): {
            "R": 0.05,
            "I": 0.7,
            "O": 0.2,
    ...
    "AB": {  # Asteroid belts
        (0.0, float("inf")): {
            ...
            "O": 0.04,
            "T": 0.01,
        },  # Mostly rocky asteroids
    },
}


def generate_moon_variety_by_parent(
    rng: SeededRandom, parent_type: str, orbital_distance_au: float
) -> str:
    """
...
def generate_moon_size(
    rng: SeededRandom,
    moon_variety: str,
    parent_mass_kg: float,
    is_orbiting_star: bool = False,
) -> Dict[str, float]:
    """
...
def generate_moon_properties(
    ...
    density_kg_m3 = calculate_density_from_composition(
        rng, composition, planet_type=None
    )
    ...
    albedo = generate_albedo(
        rng, composition, planet_type=None, has_atmosphere=has_atmosphere
    )
    ...
    if has_atmosphere and variety in ["T", "O"]:
        ...
        if is_orbiting_star:
            ...
            atmosphere = generate_atmosphere(
                rng,
                "TE" if variety == "T" else "SE",  # Approximate planet types
                mass_kg,
                radius_km,
                orbital_distance_au,
                equilibrium_temp,
            ...
            if variety == "O":
                ...
                atmosphere = generate_atmosphere(
                    ...
                    mass_kg,
                    radius_km,
                    orbital_distance_au * 10,  # Approx distance
                    equilibrium_temp,
                    star_type="G",
                )
            else:
...
# Orbital Properties
def calculate_orbital_period_days(
    orbital_distance_au: float, star_mass_kg: float
) -> float:
    """
...
def calculate_hill_sphere_radius_km(
...
def calculate_geostationary_orbit_km(
    planet_mass_kg: float, planet_radius_km: float, rotation_period_hours: float
) -> float:
    """
...
def can_retain_atmosphere(
    escape_velocity_km_s: float, temperature_k: float, molecular_mass_amu: float
) -> bool:
    """
...
def calculate_scale_height_km(
    temperature_k: float, mean_molecular_mass_amu: float, surface_gravity_m_s2: float
) -> float:
    """
...
def calculate_atmosphere_height_km(
    scale_height_km: float, surface_pressure_bar: float
) -> float:
    """
