...
class UniverseImporter:
    ...
    def import_stations(self, element: ET.Element, parent) -> None:
        ...
        for station_elem in element.findall("./station"):
            ...
            orbital_distance_km = (
                float(orbital_distance_km_text) if orbital_distance_km_text else None
            )

            orbit_type = station_elem.findtext("orbit_type") or None
            ...
            if orbit_type is not None:
                defaults["orbit_type"] = orbit_type

            station, created = Station.objects.get_or_create(
                name=name, orbits=parent, defaults=defaults
            )
            # Update fields if they exist in XML (in case station was already created)
            if not created:
                if station_elem.findtext("large_berths"):
                    station.large_berths = int(
                        station_elem.findtext("large_berths") or 0
                    )
                if station_elem.findtext("medium_berths"):
                    station.medium_berths = int(
                        station_elem.findtext("medium_berths") or 0
                    )
                if station_elem.findtext("small_berths"):
                    station.small_berths = int(
                        station_elem.findtext("small_berths") or 0
                    )
                if orbital_distance_km is not None:
                    station.orbital_distance_km = orbital_distance_km
                if orbit_type is not None:
                    station.orbit_type = orbit_type
                station.save()
    ...
    def import_universe(self) -> None:
        ...
        with transaction.atomic():
            ...
            print(
                f"- {len(controllers['control_stations'])} dedicated control/dispatch stations"
            )
            print(
                f"- {len(controllers['regular_stations'])} regular station controllers"
            )
            print(f"- {len(controllers['planets'])} planetary controllers")
            print(f"- {len(controllers['moons'])} moon controllers")
            print(
                f"Total: {sum(len(v) for v in controllers.values())} controllers deployed"
            )
    ...
    def import_planet(self, element: ET.Element, star: Star) -> Planet:
        ...
        if hasattr(planet, "orbital_period_days") and element.findtext(
            "orbital_period_days"
        ):
            planet.orbital_period_days = float(element.findtext("orbital_period_days"))
        if hasattr(planet, "rotation_period_hours") and element.findtext(
            "rotation_period_hours"
        ):
            planet.rotation_period_hours = float(
                element.findtext("rotation_period_hours")
            )
        ...
        if hasattr(planet, "is_tidally_locked") and element.findtext(
            "is_tidally_locked"
        ):
            planet.is_tidally_locked = (
                element.findtext("is_tidally_locked").lower() == "true"
            )
        if hasattr(planet, "albedo") and element.findtext("albedo"):
            planet.albedo = float(element.findtext("albedo"))
        if hasattr(planet, "equilibrium_temperature_k") and element.findtext(
            "equilibrium_temperature_k"
        ):
            planet.equilibrium_temperature_k = float(
                element.findtext("equilibrium_temperature_k")
            )
        if hasattr(planet, "orbital_eccentricity") and element.findtext(
            "orbital_eccentricity"
        ):
            planet.orbital_eccentricity = float(
                element.findtext("orbital_eccentricity")
            )
        if hasattr(planet, "orbital_inclination_deg") and element.findtext(
            "orbital_inclination_deg"
        ):
            planet.orbital_inclination_deg = float(
                element.findtext("orbital_inclination_deg")
            )
        planet.save()
        ...
        if atmosphere_elem is not None:
            ...
            if atmosphere_type and atmosphere_type != "NONE":
                ...
                atmosphere, _ = Atmosphere.objects.get_or_create(
                    ...
                    defaults={
                        ...
                        "atmosphere_height_km": float(
                            atmosphere_elem.findtext("atmosphere_height_km") or 0
                        )
                        if atmosphere_elem.findtext("atmosphere_height_km")
                        else None,
                        "surface_pressure_bar": float(
                            atmosphere_elem.findtext("surface_pressure_bar") or 0
                        )
                        if atmosphere_elem.findtext("surface_pressure_bar")
                        else None,
                        "scale_height_km": float(
                            atmosphere_elem.findtext("scale_height_km") or 0
                        )
                ...
                if not _:
                    ...
                    if atmosphere_elem.findtext("atmosphere_height_km"):
                        atmosphere.atmosphere_height_km = float(
                            atmosphere_elem.findtext("atmosphere_height_km")
                        )
                    if atmosphere_elem.findtext("surface_pressure_bar"):
                        atmosphere.surface_pressure_bar = float(
                            atmosphere_elem.findtext("surface_pressure_bar")
                        )
                    if atmosphere_elem.findtext("scale_height_km"):
                        atmosphere.scale_height_km = float(
                            atmosphere_elem.findtext("scale_height_km")
                        )
                    atmosphere.save()
    ...
    def import_moon(self, element: ET.Element, parent) -> Moon:
        ...
        if hasattr(moon, "orbital_distance_km") and element.findtext(
            "orbital_distance_km"
        ):
            moon.orbital_distance_km = float(element.findtext("orbital_distance_km"))
        if hasattr(moon, "orbital_period_hours") and element.findtext(
            "orbital_period_hours"
        ):
            moon.orbital_period_hours = float(element.findtext("orbital_period_hours"))
        if hasattr(moon, "rotation_period_hours") and element.findtext(
            "rotation_period_hours"
        ):
            moon.rotation_period_hours = float(
                element.findtext("rotation_period_hours")
            )
        ...
        if hasattr(moon, "is_tidally_locked") and element.findtext("is_tidally_locked"):
            moon.is_tidally_locked = (
                element.findtext("is_tidally_locked").lower() == "true"
            )
        if hasattr(moon, "albedo") and element.findtext("albedo"):
            moon.albedo = float(element.findtext("albedo"))
        if hasattr(moon, "equilibrium_temperature_k") and element.findtext(
            "equilibrium_temperature_k"
        ):
            moon.equilibrium_temperature_k = float(
                element.findtext("equilibrium_temperature_k")
            )
        if hasattr(moon, "orbital_eccentricity") and element.findtext(
            "orbital_eccentricity"
        ):
            moon.orbital_eccentricity = float(element.findtext("orbital_eccentricity"))
        if hasattr(moon, "orbital_inclination_deg") and element.findtext(
            "orbital_inclination_deg"
        ):
            moon.orbital_inclination_deg = float(
                element.findtext("orbital_inclination_deg")
            )
        moon.save()
