...
def create_sim_state_at_time(sim_time: float) -> SimulationState:
    ...
    sim_state = SimulationState.objects.create(
        ...
        anchor_wall_clock=current_wall_clock,
        time_scale=1.0
    )
    return sim_state
...
class TestAudioWorkerBatching:
    ...
    def test_worker_picks_soonest_actor_and_batches_their_events(self):
        ...
        jupiter = Location.objects.create(name="Jupiter", scale=Scale.PLANET)
        jupiter_control = Controller.objects.create(name="Jupiter Control")
        
        # Create another control for comparison
        saturn = Location.objects.create(name="Saturn", scale=Scale.PLANET)
...
class TestAudioWorkerCleanup:
    ...
    def test_cleanup_removes_old_files(self):
        ...
        old_event.audio_file.save('event_old.wav', ContentFile(b'old audio data'), save=True)
        ...
        # Create the file using Django's FileField
        recent_event.audio_file.save('event_recent.wav', ContentFile(b'recent audio data'), save=True)
...
class TestAudioWorkerStartupCatchup:
    ...
    def test_worker_startup_catches_up(self):
        ...
        events.append(DialogueEventLog.objects.create(
        ...
        with patch('mysite.universe.management.commands.audio_worker.ChatterboxTTSService') as MockTTS:
            ...
            processed_1 = command._process_batch(mock_tts, batch_size=3, lookahead_seconds=3600)
            assert processed_1 == 2, f"First batch should process 2 pilot events, got {processed_1}"
            
            # Second batch gets jupiter_control's event
            processed_2 = command._process_batch(mock_tts, batch_size=3, lookahead_seconds=3600)
            assert processed_2 == 1, f"Second batch should process 1 jupiter_control event, got {processed_2}"
        ...
        assert not event_too_old.audio_file, "Event beyond grace period should be skipped"
...
class TestAudioWorkerRecovery:
    ...
    def test_worker_startup_clears_stale_locks(self):
        ...
        with patch('mysite.universe.management.commands.audio_worker.ChatterboxTTSService') as MockTTS:
            ...
            processed = command._process_batch(mock_tts, batch_size=3, lookahead_seconds=3600)
    ...
    def test_worker_prioritizes_sooner_events_when_capacity_limited(self):
        ...
        soon_events = []
        for i in range(3):
            soon_events.append(DialogueEventLog.objects.create(
        ...
        later_events = []
        for i in range(3):
            later_events.append(DialogueEventLog.objects.create(
                timestamp=current_time + 2700.0 + (i * 60.0),  # T+45min, T+46min, T+47min
                actor=pilot_b,
                actor_name="Bob",
        ...
        for event in later_events:
            event.refresh_from_db()
            assert not event.audio_file, f"Later event {event.id} should not have audio yet"
...
class TestAudioWorkerCurrentTimeHandling:
    ...
    def test_worker_processes_recent_past_events(self):
        ...
        assert processed == 1, "Worker should process recent past events within grace period"
...
class TestAudioWorkerIntegration:
    ...
    def test_realistic_aba_pattern_with_lookahead_boundary(self):
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=alice, actor_name="Alice", text="Jupiter, Alice requesting clearance"))
        t += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=jupiter_control, actor_name="Jupiter Control", text="Alice, clearance granted"))
        t += 10
        events.append(DialogueEventLog.objects.create(
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=alice, actor_name="Alice", text="Jupiter, orbit established"))
        t += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=jupiter_control, actor_name="Jupiter Control", text="Alice, acknowledged"))
        t += 10
        events.append(DialogueEventLog.objects.create(
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=bob, actor_name="Bob", text="Saturn, Bob requesting docking"))
        t += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=saturn_control, actor_name="Saturn Control", text="Bob, proceed to dock"))
        t += 10
        events.append(DialogueEventLog.objects.create(
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=alice, actor_name="Alice", text="Jupiter, beginning descent"))
        t += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=jupiter_control, actor_name="Jupiter Control", text="Alice, good luck"))
        t += 10
        events.append(DialogueEventLog.objects.create(
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=bob, actor_name="Bob", text="Saturn, docking complete"))
        t += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=saturn_control, actor_name="Saturn Control", text="Bob, welcome"))
        t += 10
        events.append(DialogueEventLog.objects.create(
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=saturn_control, actor_name="Saturn Control", text="All stations, weather alert"))
        t += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=saturn_control, actor_name="Saturn Control", text="Storm approaching sector 7"))
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=t, actor=jupiter_control, actor_name="Jupiter Control", text="Alice, welcome back"))
        t += 10
        events.append(DialogueEventLog.objects.create(
        ...
        with patch('mysite.universe.management.commands.audio_worker.ChatterboxTTSService') as MockTTS:
            ...
            processed = command._process_batch(mock_tts, batch_size=3, lookahead_seconds=3600)
            assert processed == 3, "Batch 3: Should process all 3 Jupiter Control events"
            ...
            processed = command._process_batch(mock_tts, batch_size=3, lookahead_seconds=3600)
            assert processed == 3, "Batch 5: Should process first 3 Saturn Control events"
            ...
            processed = command._process_batch(mock_tts, batch_size=3, lookahead_seconds=3600)
            assert processed == 0, "Batch 8: Should have nothing left within lookahead window"
            
            # Verify events within hour are processed
            for event in within_hour:
                event.refresh_from_db()
                assert event.audio_file is not None, f"Event {event.id} ({event.actor_name} at t={event.timestamp}) should be processed"
            
            # Verify events beyond hour are NOT processed
            for event in beyond_hour:
                event.refresh_from_db()
                assert not event.audio_file, f"Event {event.id} ({event.actor_name} at t={event.timestamp}) should NOT be processed yet"
            ...
            assert total_processed_phase2 >= 2, f"Phase 2 should process at least 2 events beyond boundary, got {total_processed_phase2}"
            
            # Most events should now be processed (at least the ones within lookahead)
            processed_count = sum(1 for e in events if (e.refresh_from_db() or True) and e.audio_file is not None)
            assert processed_count >= len(within_hour), f"Should have processed at least {len(within_hour)} events"
    ...
    def test_three_missions_jupiter_control_scenario(self):
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=timestamp, actor=pilot_a, actor_name="Captain Alice",
            text="Jupiter Control, this is Captain Alice requesting clearance"
        ))
        timestamp += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=timestamp, actor=jupiter_control, actor_name="Jupiter Control",
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=timestamp, actor=pilot_b, actor_name="Captain Bob",
            text="Saturn Control, this is Captain Bob"
        ))
        timestamp += 10
        events.append(DialogueEventLog.objects.create(
            timestamp=timestamp, actor=saturn_control, actor_name="Saturn Control",
        ...
        events.append(DialogueEventLog.objects.create(
            timestamp=timestamp, actor=pilot_c, actor_name="Captain Carol",
        ...
        with patch('mysite.universe.management.commands.audio_worker.ChatterboxTTSService') as MockTTS:
            ...
            assert processed_2 == 3, "Second batch should process Jupiter Control's 3 events"
            ...
            assert processed_4 == 1, "Fourth batch should process Saturn Control's 1 event"
        ...
        for event in events:
            event.refresh_from_db()
            assert event.audio_file, f"Event {event.id} ({event.actor_name}) missing audio"
            assert event.audio_rendered_at is not None
            assert event.audio_generating is False
            assert os.path.exists(event.audio_file.path), f"File {event.audio_file.path} doesn't exist"
