...
class ActorModelTests(TestCase):
    ...
    def test_pilot_creation(self):
        ...
        with patch(
            "mysite.universe.models.actor.Pilot.generate_name",
            return_value="Captain Smith",
        ):
            pilot = Pilot.create()
    ...
    def test_controller_creation(self):
        ...
        station = Location.objects.create(name="Mars Station", scale=Scale.STATION)
        controller = Controller.create(location=station)
        self.assertIn(station.name, controller.name)
        self.assertIn(
            "You are an anonymous space traffic controller",
            controller.get_identity_prompt(),
        )
        station.delete()
    ...
    def test_satellite_creation(self):
        ...
        with patch(
            "mysite.universe.models.actor.Satellite.generate_name",
            return_value="Relay Alpha 1",
        ):
