...
def calculate_solar_angle_deg(
    system_age_years: float, orbital_period_days: float
) -> float:
...
def calculate_expected_planet_distance(
    planet1: Planet, planet2: Planet, system_age_years: float
) -> float:
    """
...
class LocationDistanceTest(TestCase):
    ...
    def test_starsystem_to_starsystem_exact(self):
        ...
        if not (
            isinstance(sol_concrete, StarSystem)
            and isinstance(binary_concrete, StarSystem)
        ):
        ...
        expected_distance = get_distance_between_star_systems(
            sol_concrete, binary_concrete
        )
        ...
        self.assertEqual(
            unit, "ly", "StarSystem-to-StarSystem distance should be in light-years"
        )
    ...
    def test_starsystem_to_starsystem_symmetry(self):
        ...
        distance_sol_binary, unit1 = get_distance_between(
            self.sol_system, self.binary_system
        )
        distance_binary_sol, unit2 = get_distance_between(
            self.binary_system, self.sol_system
        )
    ...
    def test_planet_to_planet_same_system_exact(self):
        ...
        if not (
            isinstance(earth_concrete, Planet) and isinstance(mars_concrete, Planet)
        ):
        ...
        if (
            ...
            and earth_period is not None
            and mars_period is not None
        ):
    ...
    def test_planet_to_planet_different_systems(self):
        ...
        if sol_system and binary_system:
            system_distance = get_distance_between_star_systems(
                sol_system, binary_system
            )
    ...
    def test_moon_to_moon_same_parent_exact(self):
        ...
        if not (
            isinstance(phobos_concrete, Moon) and isinstance(deimos_concrete, Moon)
        ):
    ...
    def test_moon_to_moon_symmetry(self):
        ...
        if distance_phobos_deimos is not None and distance_deimos_phobos is not None:
            self.assertAlmostEqual(
                distance_phobos_deimos,
                distance_deimos_phobos,
                places=8,
    ...
    def test_moon_to_different_planet_moon_exact(self):
        ...
        moon_km = (
            getattr(moon_concrete, "orbital_distance_km", None)
            if isinstance(moon_concrete, Moon)
            else None
        )
        phobos_km = (
            getattr(phobos_concrete, "orbital_distance_km", None)
            if isinstance(phobos_concrete, Moon)
            else None
        )
    ...
    def test_station_to_planet_symmetry(self):
        ...
        distance_station_planet, unit1 = get_distance_between(
            self.earth_control, self.mars
        )
        distance_planet_station, unit2 = get_distance_between(
            self.mars, self.earth_control
        )

        self.assertEqual(unit1, unit2, "Units should match")
        if distance_station_planet is not None and distance_planet_station is not None:
            self.assertAlmostEqual(
                distance_station_planet,
                distance_planet_station,
                places=6,
    ...
    def test_station_to_station_same_planet_exact(self):
        ...
        if not (
            isinstance(control_concrete, Station)
            and isinstance(secondary_concrete, Station)
        ):
    ...
    def test_station_to_station_different_planets_exact(self):
        ...
        control1_km = (
            getattr(earth_control_concrete, "orbital_distance_km", None)
            if isinstance(earth_control_concrete, Station)
            else None
        )
        control2_km = (
            getattr(mars_control_concrete, "orbital_distance_km", None)
            if isinstance(mars_control_concrete, Station)
            else None
        )
        ...
        expected_distance = (
            earth_mars_distance + (control1_km / 1.496e8) + (control2_km / 1.496e8)
        )
    ...
    def test_station_to_same_planet_moon_exact(self):
        ...
        if not (
            isinstance(earth_control_concrete, Station)
            and isinstance(moon_concrete, Moon)
        ):
...
class LocationServiceHierarchyTest(TestCase):
    ...
    def test_get_subordinate_bodies_location_name_not_found_returns_empty(self):
        ...
        self.assertEqual(
            get_subordinate_bodies(location_name="THIS DOES NOT EXIST"), []
        )
    ...
    def test_starsystem_subordinate_query_count_constant_in_planet_count(self):
        """
        Regression: get_subordinate_bodies() must not issue per-planet queries.
        Adding planets to the system must not change the number of queries.
        """
        from django.db import connection
        from django.test.utils import CaptureQueriesContext

        from mysite.universe.models.celestial import Star

        with CaptureQueriesContext(connection) as before:
            get_subordinate_bodies(location=self.sol_system)

        sol_star = Star.objects.get(name="Sol")
        for i in range(4):
            Planet.objects.create(name=f"QueryCount Planet {i}", orbits=sol_star)

        with CaptureQueriesContext(connection) as after:
            bodies = get_subordinate_bodies(location=self.sol_system)

        names = {b.name for b in bodies}
        self.assertIn("QueryCount Planet 0", names)
        self.assertEqual(
            len(before.captured_queries),
            len(after.captured_queries),
            "get_subordinate_bodies() query count must not grow with planet count",
        )
    ...
    def test_get_distance_between_returns_none_when_star_system_coords_missing(self):
        ...
        g = Galaxy.objects.create(
            name="Coords Galaxy", galaxy_type="SP", galaxy_size="L"
        )
    ...
    def test_same_parent_distance_returns_none_when_missing_orbital_distance(self):
        ...
        g = Galaxy.objects.create(
            name="Sibling Galaxy", galaxy_type="SP", galaxy_size="L"
        )
        ...
        s1 = Station.objects.create(
            name="S1", orbits=planet, scale=Scale.STATION, orbital_distance_km=None
        )
        s2 = Station.objects.create(
            name="S2", orbits=planet, scale=Scale.STATION, orbital_distance_km=400.0
        )
    ...
    def test_station_to_parent_uses_min_safe_orbit_fallback_when_station_missing_orbit(
        self,
    ):
        """
        ...
        g = Galaxy.objects.create(
            name="Orbit Galaxy", galaxy_type="SP", galaxy_size="L"
        )
        system = StarSystem.objects.create(
            name="Orbit System",
            orbits=g,
    ...
    def test_planet_to_planet_same_system_different_stars_falls_back_to_orbital_distance_difference(
        self,
    ):
        """
        ...
        if alpha_prime.orbits == beta_major.orbits:
            self.skipTest(
                "Planets unexpectedly orbit same star; cannot exercise different-parent fallback"
            )
    ...
    def test_station_to_parent_uses_default_orbit_when_min_safe_orbit_raises(self):
        ...
        g = Galaxy.objects.create(
            name="Error Galaxy", galaxy_type="SP", galaxy_size="L"
        )
        system = StarSystem.objects.create(
            name="Error System",
            orbits=g,
        ...
        with patch.object(
            planet, "get_min_safe_orbit_km", side_effect=RuntimeError("boom")
        ):
            d, unit = get_distance_between(station, planet)
...
class LocationServiceFallbacksTest(TestCase):
    ...
    def test_get_distance_between_different_systems_returns_none_when_system_coords_missing(
        self,
    ):
        """
        ...
        g = Galaxy.objects.create(
            name="MissingCoords Galaxy", galaxy_type="SP", galaxy_size="L"
        )
    ...
    def test_get_distance_between_star_systems_returns_inf_when_first_missing_coords(
        self,
    ):
        ...
        self.assertEqual(
            get_distance_between_star_systems(sys_missing, sys_with), float("inf")
        )
    ...
    def test_planet_to_planet_same_star_falls_back_when_orbital_period_missing(self):
        ...
        g = Galaxy.objects.create(
            name="Period Galaxy", galaxy_type="SP", galaxy_size="L"
        )
        system = StarSystem.objects.create(
            name="Period System",
            orbits=g,
    ...
    def test_calculate_same_parent_distance_prefers_orbital_distance_au(self):
        ...
        p1 = Planet.objects.create(
            name="Tri P1", orbits=star, planet_type="TE", orbital_distance_au=1.0
        )
        p2 = Planet.objects.create(
            name="Tri P2", orbits=star, planet_type="TE", orbital_distance_au=2.0
        )
        ...
        expected = math.sqrt(
            1.0**2 + 2.0**2 - 2 * 1.0 * 2.0 * math.cos(math.radians(60.0))
        )
    ...
    def test_get_orbital_distance_from_parent_converts_km_to_au_for_moon(self):
        ...
        planet = Planet.objects.create(
            name="Moon Parent", orbits=star, planet_type="TE", orbital_distance_au=1.0
        )
        moon = Moon.objects.create(
            name="Moon Child",
            orbits=planet,
            moon_type="R",
            orbital_distance_km=384400.0,
        )
    ...
    def test_get_orbital_parent_returns_orbits_for_orbiting_objects(self):
        ...
        g = Galaxy.objects.create(
            name="Parent Galaxy", galaxy_type="SP", galaxy_size="L"
        )
        system = StarSystem.objects.create(
            name="Parent System",
            orbits=g,
        ...
        planet = Planet.objects.create(
            name="Parent Planet", orbits=star, planet_type="TE", orbital_distance_au=1.0
        )
    ...
    def test_get_orbital_distance_from_parent_returns_au_for_planet(self):
        ...
        planet = Planet.objects.create(
            name="AU Planet", orbits=star, planet_type="TE", orbital_distance_au=1.234
        )

        self.assertAlmostEqual(
            _get_orbital_distance_from_parent(planet, star), 1.234, places=12
        )
    ...
    def test_calculate_planet_to_planet_distance_returns_none_when_system_is_not_starsystem(
        self,
    ):
        ...
        not_a_system = Location.objects.create(
            name="NotAStarSystem", scale=Scale.STATION
        )
    ...
    def test_get_orbital_distance_from_parent_returns_none_for_star(self):
        ...
        g = Galaxy.objects.create(
            name="NoOrbit Galaxy", galaxy_type="SP", galaxy_size="L"
        )
        system = StarSystem.objects.create(
            name="NoOrbit System",
            orbits=g,
