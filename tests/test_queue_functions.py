...
# ---------------------------------------------------------------------------
# SimulationQueue edge cases not yet covered
# ---------------------------------------------------------------------------


def test_get_next_event_on_empty_queue_returns_none():
    """get_next_event() on an empty queue returns None without raising."""
    queue = SimulationQueue()
    assert queue.get_next_event() is None


def test_process_due_events_swallows_signal_exception():
    """An exception raised inside signal processing is caught; remaining events still run."""
    queue = SimulationQueue()
    dummy = DummyActor()
    queue.add_event(
        DialogueEvent(
            timestamp=0.0, actor=dummy, text="A", duration=1.0, event_type="dialogue"
        )
    )
    queue.add_event(
        DialogueEvent(
            timestamp=0.0, actor=dummy, text="B", duration=1.0, event_type="dialogue"
        )
    )

    with patch(
        "mysite.universe.management.commands.start_simulation_loop.dialogue_event_processed.send",
        side_effect=RuntimeError("boom"),
    ):
        # Must not propagate; both events are consumed
        queue.process_due_events(1.0)
...
class TestDemoQueue(TestCase):
    """DemoQueue processes all events with optional per-event delays."""

    def setUp(self):
        self.dummy = DummyActor()

    def _make_queue(self, n=3, delay=0.0) -> DemoQueue:
        q = DemoQueue(delay_seconds=delay)
        for i in range(n):
            q.add_event(
                DialogueEvent(
                    timestamp=float(i),
    ...
    def test_init_stores_delay(self):
        """DemoQueue stores the delay_seconds passed to __init__."""
        q = DemoQueue(delay_seconds=2.5)
        self.assertEqual(q.delay_seconds, 2.5)

    def test_init_default_delay_is_five(self):
        """Default delay_seconds is 5.0."""
        q = DemoQueue()
        self.assertEqual(q.delay_seconds, 5.0)

    def test_process_all_events_returns_count(self):
        q = self._make_queue(n=3)
        count = q.process_all_events()
        self.assertEqual(count, 3)

    def test_process_all_events_empties_queue(self):
        q = self._make_queue(n=2)
        q.process_all_events()
        self.assertIsNone(q.peek_next_event())

    def test_process_all_events_calls_callback(self):
        q = self._make_queue(n=3)
        seen = []
        q.process_all_events(callback=lambda e: seen.append(e.text))
        self.assertEqual(seen, ["Event 0", "Event 1", "Event 2"])

    def test_process_all_events_sleeps_between_events(self):
        """time.sleep is called between events when delay > 0."""
        q = self._make_queue(n=3, delay=1.0)
        with patch("time.sleep") as mock_sleep:
            q.process_all_events()
        # Sleep called between each pair of events: 2 times for 3 events
        self.assertEqual(mock_sleep.call_count, 2)

    def test_process_all_events_no_sleep_when_delay_zero(self):
        q = self._make_queue(n=3, delay=0.0)
        with patch("time.sleep") as mock_sleep:
            q.process_all_events()
        mock_sleep.assert_not_called()

    def test_process_all_events_swallows_signal_exception(self):
        """Exception during signal emission is caught; loop continues."""
        q = self._make_queue(n=2)
        with patch(
            "mysite.universe.management.commands.start_simulation_loop.dialogue_event_processed.send",
            ...
            count = q.process_all_events()
        # Both events attempted even though both raised
        self.assertEqual(count, 0)  # events_processed only incremented on success
        self.assertIsNone(q.peek_next_event())

    def test_process_all_events_instant_restores_delay(self):
        """process_all_events_instant() resets delay_seconds to its original value."""
        q = self._make_queue(n=2, delay=3.0)
        with patch("time.sleep"):
            q.process_all_events_instant()
        self.assertEqual(q.delay_seconds, 3.0)

    def test_process_all_events_instant_returns_count(self):
        q = self._make_queue(n=4)
        count = q.process_all_events_instant()
        self.assertEqual(count, 4)

    def test_process_all_events_instant_does_not_sleep(self):
        q = self._make_queue(n=3, delay=99.0)
        with patch("time.sleep") as mock_sleep:
            q.process_all_events_instant()
        mock_sleep.assert_not_called()

    def test_process_all_events_handles_navigation_event(self):
        """NavigationEvent branch in DemoQueue.process_all_events() emits nav signal."""
        q = DemoQueue(delay_seconds=0.0)
        q.add_event(
            NavigationEvent(
                timestamp=0.0,
                maneuver=ManeuverType.LAUNCH,
        ...
        received = []
        with patch(
            "mysite.universe.management.commands.start_simulation_loop.navigation_event_processed.send",
            side_effect=lambda sender, event: received.append(event),
        ):
            count = q.process_all_events()
...
class TestCommandEdgeCases(TestCase):
    """Covers Command.handle(), load_test_events(), and KeyboardInterrupt path."""

    def setUp(self):
        self.cmd = Command()
        self.cmd.stdout = MagicMock()

    def test_load_initial_events_is_noop(self):
        """load_initial_events() exists and does nothing (pass body)."""
        queue = SimulationQueue()
        self.cmd.load_initial_events(queue)  # must not raise, queue unchanged
        self.assertIsNone(queue.peek_next_event())

    @pytest.mark.django_db
    def test_load_test_events_adds_three_events(self):
        """load_test_events() creates Pilot/Controller in DB and loads 3 events."""
        queue = SimulationQueue()
        self.cmd.load_test_events(queue)
        # Three events should be in the queue
        count = 0
    ...
    @pytest.mark.django_db
    def test_load_test_events_idempotent_pilot(self):
        """Calling load_test_events() twice reuses the existing Pilot."""
        from mysite.universe.models.actor import Pilot

        queue = SimulationQueue()
        self.cmd.load_test_events(queue)
        self.cmd.load_test_events(SimulationQueue())
        pilot_count = Pilot.objects.filter(name="Test Pilot").count()
        self.assertEqual(pilot_count, 1)

    def test_start_simulation_loop_keyboard_interrupt_is_handled(self):
        """KeyboardInterrupt raised by time.sleep is caught and prints a message.

        Event is at t=5; time_fn always returns 100.0 so current_time=0.0 and
        the event is never due. The loop reaches time.sleep() which raises the
        interrupt before any signal or DB write occurs.
        """
        queue = SimulationQueue()
        dummy = DummyActor()
        queue.add_event(
        ...
        with patch("time.sleep", side_effect=KeyboardInterrupt):
            self.cmd.start_simulation_loop(queue, time_fn=lambda: 100.0)
        self.cmd.stdout.write.assert_any_call("Simulation stopped")

    def test_handle_calls_start_simulation_loop(self):
        """handle() wires together load_test_events and start_simulation_loop."""
        with patch.object(self.cmd, "load_test_events") as mock_load:
            with patch.object(self.cmd, "start_simulation_loop") as mock_loop:
                self.cmd.handle()
