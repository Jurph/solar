...
class TestRouteMissions(TestCase):
    def setUp(self):
        ...
        self.ctrl_origin = Location.objects.create(
            name="Origin Control", scale=Scale.STATION
        )
        self.ctrl_dest = Location.objects.create(
            name="Dest Control", scale=Scale.STATION
        )
    ...
    def test_pick_random_destination_filters_by_max_scale(self):
        ...
        with patch(
            "mysite.universe.services.route.missions.random.choice",
    ...
    def test_pick_random_destination_raises_when_no_eligible(self):
        ...
        with self.assertRaises(ValueError):
            pick_random_destination(
                service=None, excluding=self.origin, cargo_mission=False
            )
    ...
    def test_pick_random_destination_cargo_uses_ship_valid_locations(self):
        ...
        with (
            patch.object(
                Ship, "get_valid_cargo_locations", return_value=[self.origin, other]
            ),
            patch(
                "mysite.universe.services.route.missions.random.choice",
                side_effect=lambda xs: xs[0],
            ...
            picked = pick_random_destination(
                service=None, excluding=self.origin, cargo_mission=True
            )
        assert picked == other
    ...
    def test_random_journey_assigns_origin_when_ship_location_missing(self):
        ...
        with (
            ...
            patch(
                "mysite.universe.services.route.missions.pick_random_destination",
                return_value=self.dest,
            ),
            patch(
                "mysite.universe.services.route.missions.UniverseGraph.get_instance",
    ...
    def test_random_journey_preserves_existing_controller(self):
        ...
        service = _DummyService(
            events=[existing], controller_for=lambda loc: self.ctrl_dest
        )
    ...
    def test_get_local_locations_delegates_to_universe_graph(self):
        ...
        with patch(
            ...
            locals_ = get_local_locations(
                service=None, current=current, max_scale=Scale.STATION
            )
