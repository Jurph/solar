...
class TestProceduralGenerationCoverage(TestCase):
    ...
    def test_distribution_and_hash_helpers_are_bounded_and_deterministic(self):
        ...
        assert 0.0 <= normal_clamped(rng, mu=100.0, sigma=1000.0, min_val=0.0, max_val=1.0) <= 1.0
        assert 1.0 <= log_normal_clamped(rng, mu=0.0, sigma=1.0, min_val=1.0, max_val=10.0) <= 10.0

        val = bin_select(rng, bins=[(0.0, 1.0), (10.0, 11.0)])
        assert (0.0 <= val < 1.0) or (10.0 <= val < 11.0)

        val2 = bin_select_weighted(rng, bins=[(0.0, 1.0), (10.0, 11.0)], weights=[0.0, 1.0])
    ...
    def test_planet_type_generation_handles_m_dwarf_adjustment_and_nan_fallback(self):
        ...
        fallback_type = generate_planet_type(rng2, orbital_distance_au=float("nan"), star_type="G")
    ...
    def test_density_from_composition_covers_gas_giant_and_surface_effects(self):
        ...
        gg_density = calculate_density_from_composition(rng, base_comp, planet_type="GG")
        assert 200 <= gg_density <= 20000

        rng_ig = SeededRandom(1001, name="dens_ig")
        ig_density = calculate_density_from_composition(rng_ig, base_comp, planet_type="IG")
        ...
        with patch.object(rng2, "uniform", return_value=0.0):
            d_no_methane = calculate_density_from_composition(rng2, {**base_comp, "has_methane": False}, planet_type="TE")
            d_methane = calculate_density_from_composition(rng2, {**base_comp, "has_methane": True}, planet_type="TE")
            assert d_methane < d_no_methane

            d_dry = calculate_density_from_composition(rng2, {**base_comp, "water_coverage": 0.0}, planet_type="TE")
            d_wet = calculate_density_from_composition(rng2, {**base_comp, "water_coverage": 1.0}, planet_type="TE")
            assert d_wet < d_dry

            # Cover the Cthonian variation branch (narrower variation band)
            d_ct = calculate_density_from_composition(rng2, {**base_comp, "water_coverage": 0.0}, planet_type="CT")
            assert 200 <= d_ct <= 20000

    def test_generate_albedo_covers_gas_giant_ice_giant_and_atmosphere_boost(self):
        from unittest.mock import patch

        comp = {"ice_content": 0.0, "water_coverage": 0.0, "iron_content": 0.0, "carbon_content": 0.0, "organic_haze": 0.0}
        rng = SeededRandom(123, name="alb")
        assert 0.01 <= generate_albedo(rng, comp, planet_type="GG", has_atmosphere=False) <= 0.99
        assert 0.01 <= generate_albedo(rng, comp, planet_type="IG", has_atmosphere=False) <= 0.99
        ...
        with patch.object(rng2, "uniform", side_effect=_uniform):
            no_atmo = generate_albedo(rng2, comp, planet_type="TE", has_atmosphere=False)
            with_atmo = generate_albedo(rng2, comp, planet_type="TE", has_atmosphere=True)
    ...
    def test_moon_quantity_and_variety_edge_cases(self):
        ...
        v = generate_moon_variety_by_parent(rng2, parent_type="STAR", orbital_distance_au=float("inf"))
        assert v in {"R", "I", "O", "T"}

        # Unknown parent => defaults to rocky
        assert generate_moon_variety_by_parent(rng2, parent_type="NOT_A_PARENT", orbital_distance_au=1.0) == "R"
    ...
    def test_moon_size_caps_and_radius_adjustments(self):
        ...
        with patch.object(rng, "uniform", side_effect=[0.02, 0.2]):
            props = generate_moon_size(rng, moon_variety="R", parent_mass_kg=1.0e24, is_orbiting_star=True)
        ...
        with patch.object(rng2, "uniform", side_effect=[0.0001, 0.3]):
            props2 = generate_moon_size(rng2, moon_variety="R", parent_mass_kg=1.0e30, is_orbiting_star=False)
        ...
        with patch.object(rng3, "uniform", side_effect=[0.02, 0.1]):
            props3 = generate_moon_size(rng3, moon_variety="R", parent_mass_kg=1.0e30, is_orbiting_star=False)
        assert props3["radius_km"] >= 0.1 * 6371

        # Unknown variety should default to 'R'
        rng4 = SeededRandom(5, name="ms4")
        props4 = generate_moon_size(rng4, moon_variety="???", parent_mass_kg=1.0e30, is_orbiting_star=False)
    ...
    def test_generate_moon_properties_can_generate_atmosphere_in_both_paths(self):
        ...
        with (
            patch("mysite.universe.procedural_generation.generate_moon_variety_by_parent", return_value="T"),
    ...
    def test_should_generate_moons_for_parent(self):
        ...
        planet = Planet.objects.create(name="SG Planet", orbits=star, planet_type="TE", orbital_distance_au=1.0)
    ...
    def test_orbital_and_atmosphere_physics_helpers(self):
        # solar angle
        angle = calculate_solar_angle_deg(system_age_years=4.6e9, orbital_period_days=365.25)
        assert 0.0 <= angle < 360.0

        # hill sphere should shrink with eccentricity
        r0 = calculate_hill_sphere_radius_km(planet_mass_kg=5.972e24, star_mass_kg=1.989e30, orbital_distance_au=1.0, eccentricity=0.0)
        r1 = calculate_hill_sphere_radius_km(planet_mass_kg=5.972e24, star_mass_kg=1.989e30, orbital_distance_au=1.0, eccentricity=0.5)
        assert r1 < r0

        assert is_tidally_locked(orbital_distance_au=0.01, planet_mass_kg=5.972e24, planet_radius_km=6371, star_mass_kg=1.989e30, system_age_years=1e6) is True
        assert is_tidally_locked(orbital_distance_au=0.1, planet_mass_kg=5.972e24, planet_radius_km=6371, star_mass_kg=1.989e30, system_age_years=2e9) is True
        assert is_tidally_locked(orbital_distance_au=0.4, planet_mass_kg=1.0e24, planet_radius_km=3000, star_mass_kg=1.989e30, system_age_years=6e9) is True
        assert is_tidally_locked(orbital_distance_au=1.0, planet_mass_kg=5.972e24, planet_radius_km=6371, star_mass_kg=1.989e30, system_age_years=1e9) is False

        # geostationary orbit (Earth) ~42164 km from center
        geo = calculate_geostationary_orbit_km(planet_mass_kg=5.972e24, planet_radius_km=6371, rotation_period_hours=23.934)
        assert 40000 <= geo <= 45000

        esc = calculate_escape_velocity_km_s(mass_kg=5.972e24, radius_km=6371)
        assert 10.0 <= esc <= 12.0

        assert can_retain_atmosphere(escape_velocity_km_s=11.2, temperature_k=288, molecular_mass_amu=28.0) is True
        assert can_retain_atmosphere(escape_velocity_km_s=1.0, temperature_k=500, molecular_mass_amu=28.0) is False
        assert can_retain_atmosphere(escape_velocity_km_s=11.2, temperature_k=288, molecular_mass_amu=0.0) is False

        # atmosphere decision tree: exercise several branches
        rng = SeededRandom(77, name="atmo")
        assert determine_atmosphere_type(rng, "GG", orbital_distance_au=5.0, escape_velocity_km_s=50.0, temperature_k=200, star_type="G") == "H2_HE"
        assert determine_atmosphere_type(rng, "IG", orbital_distance_au=5.0, escape_velocity_km_s=50.0, temperature_k=200, star_type="G") == "H2_HE"
        assert determine_atmosphere_type(rng, "AB", orbital_distance_au=3.0, escape_velocity_km_s=1.0, temperature_k=200, star_type="G") == "NONE"
        assert determine_atmosphere_type(rng, "CT", orbital_distance_au=1.0, escape_velocity_km_s=20.0, temperature_k=500, star_type="G") == "NONE"
        assert determine_atmosphere_type(rng, "TE", orbital_distance_au=0.01, escape_velocity_km_s=2.0, temperature_k=2000, star_type="G") == "NONE"

        # Very hot branch
        assert determine_atmosphere_type(rng, "TE", orbital_distance_au=0.5, escape_velocity_km_s=9.0, temperature_k=1600, star_type="G") == "CO2_THIN"
        assert determine_atmosphere_type(rng, "TE", orbital_distance_au=0.5, escape_velocity_km_s=7.0, temperature_k=1600, star_type="G") == "NONE"

        # Steam atmosphere branch
        assert determine_atmosphere_type(rng, "TE", orbital_distance_au=1.0, escape_velocity_km_s=30.0, temperature_k=1200, star_type="G") == "H2O"

        # Close-in photoevaporation branch (<0.5 AU) with CO2 retention
        close = determine_atmosphere_type(rng, "SE", orbital_distance_au=0.2, escape_velocity_km_s=15.0, temperature_k=500, star_type="G")
        assert close in {"CO2_THICK", "CO2_THIN"}

        # Very distant outer-system branch (>10 AU)
        assert determine_atmosphere_type(rng, "TE", orbital_distance_au=20.0, escape_velocity_km_s=6.0, temperature_k=50, star_type="G") == "N2_CH4"
        assert determine_atmosphere_type(rng, "TE", orbital_distance_au=20.0, escape_velocity_km_s=4.0, temperature_k=50, star_type="G") == "NONE"

        # scale height / atmosphere height
        H = calculate_scale_height_km(temperature_k=288, mean_molecular_mass_amu=29.0, surface_gravity_m_s2=9.81)
        assert H > 0.0
        assert calculate_scale_height_km(temperature_k=288, mean_molecular_mass_amu=0.0, surface_gravity_m_s2=9.81) == 0.0

        assert calculate_atmosphere_height_km(scale_height_km=7.0, surface_pressure_bar=0.001) == 42.0
        assert calculate_atmosphere_height_km(scale_height_km=7.0, surface_pressure_bar=1.0) == 56.0
        assert calculate_atmosphere_height_km(scale_height_km=7.0, surface_pressure_bar=50.0) == 70.0
        assert calculate_atmosphere_height_km(scale_height_km=0.0, surface_pressure_bar=1.0) == 0.0
    ...
    def test_color_palette_generation_helpers(self):
        ...
        ocean = generate_color_palette_from_composition({"water_coverage": 0.9}, temperature_k=300)
        ice = generate_color_palette_from_composition({"ice_content": 0.9}, temperature_k=100)
        iron = generate_color_palette_from_composition({"iron_content": 0.9}, temperature_k=300)
