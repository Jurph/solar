...
class EventFeedAudioReadyFlagTests(TestCase):
    ...
    def test_event_feed_audio_ready_flag_matches_file_existence(self):
        ...
        event_ready.audio_file.save(f'event_{event_ready.id}.wav', 
                                    ContentFile(wav_bytes))
        ...
        event_pending_data = next(e for e in data['events'] 
                                 if e['id'] == event_pending.id)
        event_ready_data = next(e for e in data['events'] 
                               if e['id'] == event_ready.id)
        
        # Verify audio_ready flags
        self.assertFalse(event_pending_data.get('audio_ready', False),
            "Event without audio_file should have audio_ready=false")
        self.assertTrue(event_ready_data.get('audio_ready', False),
